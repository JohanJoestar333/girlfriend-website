   ...
   ================================================================ */
      const CONFIG = {
        ...
        // ---- TURN relay (for the Photo Booth when you're on different
        // networks/wifis) ----
        // The Photo Booth video call uses WebRTC, which usually needs a
        // "TURN" relay server to connect two people who are NOT on the same
        // wifi (e.g. one of you on home wifi, the other on mobile data).
        // Without one, the booth works fine when you're together on the same
        // wifi, but shows a black screen / "Partner disconnected or the
        // connection failed" message the moment you're on different networks.
        // This project used to ship with a shared public TURN server
        // (openrelay.metered.ca) that anyone could use for free with no
        // signup — that shared server has since become unreliable, which is
        // almost certainly why the booth stopped working across networks.
        // Getting your OWN free TURN credentials (5 minutes, no credit card)
        // fixes this for good. Full walkthrough: see METERED_TURN_SETUP.md.
        //   1. Go to https://dashboard.metered.ca/signup and create a free account.
        //   2. Click "Add Application", give it any name, and open it.
        //   3. Copy the "App Domain" (looks like yourappname.metered.live)
        //      and the "Secret Key" shown on that app's page.
        // Leave both as "__METERED_TURN_DOMAIN__" / "__METERED_TURN_API_KEY__"
        // and the booth still works on the same wifi — it just can't
        // connect when you're apart.
        meteredTurn: {
        ...
        // ---- TMDB (The Movie Database) — powers movie search in the Bucket
        // List's "Movie Nights" section. Free, no credit card required:
        //   1. Create an account at https://www.themoviedb.org and verify your email.
        //   2. Go to Settings > API > click "Create" and fill the short form
        //      (choose "Developer", personal/non-commercial use is fine).
        //   3. Copy the "API Read Access Token" (the long token) from your
        //      TMDB account's API settings and keep it in the GitHub secret
        //      named TMDB_ACCESS_TOKEN. The deploy workflow injects it below.
        tmdb: {
   ...
   ================================================================ */
      ...
      const CHANGE_TOAST_CONFIG = {
        bucketlist: {
          ...
          en: (n) =>
            n === 1
              ? "A Bucket List item changed"
              : n + " Bucket List items changed",
          pt: (n) =>
            n === 1
        ...
        music: {
          ...
          en: (n) =>
            n === 1
              ? "A playlist song changed"
              : n + " playlist songs changed",
          pt: (n) =>
            n === 1
        ...
        album: {
          ...
          en: (n) =>
            n === 1
              ? "An Album memory changed"
              : n + " Album memories changed",
          pt: (n) =>
            n === 1
      ...
      function refreshTabDotsFromData(tab, items) {
        ...
        const unseen = (items || []).filter((it) => {
          ...
          return changed > last && changed > 0 && now - changed <= CHANGE_NOTIFICATION_TTL;
        });
        if (unseen.length && !isSelfWriteWindow(tab)) {
          const cfg = CHANGE_TOAST_CONFIG[tab];
          if (cfg) {
            const message = currentLanguage() === "pt" ? cfg.pt(unseen.length) : cfg.en(unseen.length);
            showToast(cfg.icon + " " + message, "change-notification", CHANGE_TOAST_DURATION);
          }
        }
        markTabSeen(tab, items);
      }
      ...
      const FALLBACK_QUOTES = [
        ...
        { q: "When you are content to be simply yourself and don't compare or compete, everyone will respect you.", a: "Lao Tzu" },
        ...
        { q: "He who knows that enough is enough will always have enough.", a: "Lao Tzu" },
        { q: "A journey of a thousand miles begins beneath one's feet.", a: "Lao Tzu" },
        { q: "The sage does not hoard. The more he helps others, the more he benefits himself.", a: "Lao Tzu" },
        { q: "The softest things in the world overcome the hardest things in the world.", a: "Lao Tzu" },
        { q: "To attain knowledge, add things every day. To attain wisdom, remove things every day.", a: "Lao Tzu" },
        { q: "Mastering others is strength; mastering yourself is true power.", a: "Lao Tzu" },
        { q: "Do the difficult things while they are easy and do the great things while they are small.", a: "Lao Tzu" },
        { q: "If you do not change direction, you may end up where you are heading.", a: "Lao Tzu" },
        { q: "A man who asks is a fool for five minutes. A man who never asks is a fool for life.", a: "Chinese proverb" },
        ...
        { q: "Even a journey of a thousand miles begins with a single step.", a: "Chinese proverb" },
        { q: "What you do not want done to yourself, do not do to others.", a: "Confucius" },
        ...
        { q: "To see and listen to the wicked is already the beginning of wickedness.", a: "Confucius" },
        ...
        { q: "For every minute you are angry you lose sixty seconds of happiness.", a: "Ralph Waldo Emerson" },
        ...
        { q: "The reward of a thing well done is having done it.", a: "Ralph Waldo Emerson" },
        { q: "Once you make a decision, the universe conspires to make it happen.", a: "Ralph Waldo Emerson" },
        { q: "The creation of a thousand forests is in one acorn.", a: "Ralph Waldo Emerson" },
        { q: "Life consists in what a man is thinking of all day.", a: "Ralph Waldo Emerson" },
        { q: "Earth laughs in flowers.", a: "Ralph Waldo Emerson" },
        { q: "All life is an experiment. The more experiments you make the better.", a: "Ralph Waldo Emerson" },
        { q: "Write it on your heart that every day is the best day in the year.", a: "Ralph Waldo Emerson" },
        { q: "Beauty without expression is boring.", a: "Ralph Waldo Emerson" },
        { q: "Hitch your wagon to a star.", a: "Ralph Waldo Emerson" },
        { q: "The purpose of life is not to be happy. It is to be useful, honorable, compassionate.", a: "Ralph Waldo Emerson" },
        ...
        { q: "The years teach much which the days never know.", a: "Ralph Waldo Emerson" },
        { q: "It is not length of life, but depth of life.", a: "Ralph Waldo Emerson" },
        { q: "The mass of men lead lives of quiet desperation.", a: "Henry David Thoreau" },
        { q: "Go confidently in the direction of your dreams. Live the life you have imagined.", a: "Henry David Thoreau" },
        { q: "How vain it is to sit down to write when you have not stood up to live.", a: "Henry David Thoreau" },
        { q: "All misfortune is but a stepping stone to fortune.", a: "Henry David Thoreau" },
        { q: "Goodness is the only investment that never fails.", a: "Henry David Thoreau" },
        { q: "Simplify, simplify.", a: "Henry David Thoreau" },
        { q: "Could a greater miracle take place than for us to look through each other's eyes for an instant?", a: "Henry David Thoreau" },
        { q: "The price of anything is the amount of life you exchange for it.", a: "Henry David Thoreau" },
        { q: "Things do not change; we change.", a: "Henry David Thoreau" },
        { q: "Live deliberately.", a: "Henry David Thoreau" },
        { q: "The fault-finder will find faults even in paradise.", a: "Henry David Thoreau" },
        { q: "An early-morning walk is a blessing for the whole day.", a: "Henry David Thoreau" },
        { q: "What you get by achieving your goals is not as important as what you become by achieving your goals.", a: "Henry David Thoreau" },
        { q: "Rather than love, than money, than fame, give me truth.", a: "Henry David Thoreau" },
        { q: "Disobedience is the true foundation of liberty.", a: "Henry David Thoreau" },
        { q: "If a man does not keep pace with his companions, perhaps it is because he hears a different drummer.", a: "Henry David Thoreau" },
        { q: "Heaven is under our feet as well as over our heads.", a: "Henry David Thoreau" },
        { q: "The question is not what you look at, but what you see.", a: "Henry David Thoreau" },
        { q: "As if you could kill time without injuring eternity.", a: "Henry David Thoreau" },
        { q: "A man is rich in proportion to the number of things which he can afford to let alone.", a: "Henry David Thoreau" },
        { q: "The best thing a man can do for his culture when he is rich is to endeavor to carry out those schemes which when he was poor he formed.", a: "Henry David Thoreau" },
        { q: "If you have built castles in the air, your work need not be lost; that is where they should be.", a: "Henry David Thoreau" },
        { q: "The only way to tell the truth is to speak with kindness.", a: "Leo Tolstoy" },
        ...
        { q: "There is no greatness where there is no simplicity, goodness, and truth.", a: "Leo Tolstoy" },
        { q: "One of the first conditions of happiness is that the link between man and nature shall not be broken.", a: "Leo Tolstoy" },
        { q: "We can know only that we know nothing. And that is the highest degree of human wisdom.", a: "Leo Tolstoy" },
        { q: "Wrong does not cease to be wrong because the majority share in it.", a: "Leo Tolstoy" },
        { q: "In the name of God, stop a moment, cease your work, look around you.", a: "Leo Tolstoy" },
        { q: "Art is not a handicraft, it is the transmission of feeling the artist has experienced.", a: "Leo Tolstoy" },
        ...
        { q: "Part of the secret of a success in life is to eat what you like and let the food fight it out inside.", a: "Mark Twain" },
        ...
        { q: "Truth is stranger than fiction, but it is because Fiction is obliged to stick to possibilities; Truth isn't.", a: "Mark Twain" },
        { q: "The difference between the right word and the almost right word is the difference between lightning and a lightning bug.", a: "Mark Twain" },
        { q: "If you tell the truth, you don't have to remember anything.", a: "Mark Twain" },
        { q: "Give every day the chance to become the most beautiful day of your life.", a: "Mark Twain" },
        { q: "To succeed in life, you need two things: ignorance and confidence.", a: "Mark Twain" },
        { q: "The human race has one really effective weapon, and that is laughter.", a: "Mark Twain" },
        { q: "A successful man is one who makes more money than his wife can spend. A successful woman is one who can find such a man.", a: "Mark Twain" },
        { q: "Never put off till tomorrow what may be done day after tomorrow just as well.", a: "Mark Twain" },
        { q: "I have never let my schooling interfere with my education.", a: "Mark Twain" },
        { q: "The fear of death follows from the fear of life. A man who lives fully is prepared to die at any time.", a: "Mark Twain" },
        { q: "Forgiveness is the fragrance that the violet sheds on the heel that has crushed it.", a: "Mark Twain" },
        ...
        { q: "O, may I join the choir invisible of those immortal dead who live again in minds made better by their presence.", a: "George Eliot" },
        { q: "Blessed is he who, when there is no good to be done, has no evil to do.", a: "George Eliot" },
        { q: "What do we live for, if it is not to make life less difficult for each other?", a: "George Eliot" },
        { q: "Animals are such agreeable friends—they ask no questions; they pass no criticisms.", a: "George Eliot" },
        { q: "The strongest principle of growth lies in human choice.", a: "George Eliot" },
        { q: "Our deeds still travel with us from afar, and what we have been makes us what we are.", a: "George Eliot" },
        { q: "A difference of taste in jokes is a great strain on the affections.", a: "George Eliot" },
        { q: "Life began with waking up and loving my mother's face.", a: "George Eliot" },
        { q: "Don't judge each day by the harvest you reap but by the seeds that you plant.", a: "Robert Louis Stevenson" },
        { q: "Keep your fears to yourself, but share your courage with others.", a: "Robert Louis Stevenson" },
        { q: "Our business in life is not to succeed, but to continue to fail in good spirits.", a: "Robert Louis Stevenson" },
        { q: "The most beautiful adventures are often the most dangerous.", a: "Robert Louis Stevenson" },
        { q: "Sooner or later everyone sits down to a banquet of consequences.", a: "Robert Louis Stevenson" },
        { q: "To be wholly devoted to some intellectual exercise is to have succeeded in life.", a: "Robert Louis Stevenson" },
        { q: "There is so much good in the worst of us, and so much bad in the best of us.", a: "Robert Louis Stevenson" },
        { q: "We are all travelers in the wilderness of this world, and the best we can find in our travels is an honest friend.", a: "Robert Louis Stevenson" },
        { q: "Keep your fears to yourself, but share your inspiration with others.", a: "Robert Louis Stevenson" },
        { q: "The world is full of a number of things, I'm sure we should all be as happy as kings.", a: "Robert Louis Stevenson" },
        { q: "Each day is a little life: every waking and rising a little birth, every fresh morning a little youth.", a: "Arthur Schopenhauer" },
        { q: "All truth passes through three stages. First it is ridiculed. Second it is opposed. Third it is regarded as self-evident.", a: "Arthur Schopenhauer" },
        { q: "Compassion is the basis of morality.", a: "Arthur Schopenhauer" },
        { q: "The greatest of follies is to sacrifice health for any other type of happiness.", a: "Arthur Schopenhauer" },
        { q: "Every man takes the limits of his own field of vision for the limits of the world.", a: "Arthur Schopenhauer" },
        { q: "It is difficult to find happiness within oneself, but it is impossible to find it anywhere else.", a: "Arthur Schopenhauer" },
        { q: "The two enemies of human happiness are pain and boredom.", a: "Arthur Schopenhauer" },
        ...
        { q: "Opposition brings concord. Out of discord comes the fairest harmony.", a: "Heraclitus" },
        ...
        { q: "Music gives a soul to the universe, wings to the mind, flight to the imagination and life to everything.", a: "Plato" },
        { q: "One of the penalties for refusing to participate in politics is that you end up being governed by your inferiors.", a: "Plato" },
        { q: "The greatest wealth is to live content with little.", a: "Plato" },
        { q: "Courage is the first of human qualities because it is the quality which guarantees the others.", a: "Aristotle" },
        ...
        { q: "It is well to be up before daybreak, for such habits contribute to health, wealth, and wisdom.", a: "Aristotle" },
        { q: "Dignity does not consist in possessing honors, but in deserving them.", a: "Aristotle" },
        { q: "Hope is a waking dream.", a: "Aristotle" },
        { q: "My best skill was that I was teachable.", a: "Aristotle" },
        { q: "What is a friend? A single soul dwelling in two bodies.", a: "Aristotle" },
        ...
        { q: "The good of the community cannot be achieved without the good of individuals.", a: "Aristotle" },
        ...
        { q: "Very little is needed to make a happy life; it is all within yourself, your way of thinking.", a: "Marcus Aurelius" },
        ...
        { q: "The things you think about determine the quality of your mind.", a: "Marcus Aurelius" },
        ...
        { q: "Do every act of your life as though it were the last act of your life.", a: "Marcus Aurelius" },
        { q: "Think of yourself as dead. You have lived your life. Now take what's left and live it properly.", a: "Marcus Aurelius" },
        { q: "You should banish any thought of how you appear to others.", a: "Marcus Aurelius" },
        { q: "The only wealth you keep forever is the wealth you give away.", a: "Marcus Aurelius" },
        { q: "A man's worth is no greater than his ambitions.", a: "Marcus Aurelius" },
        { q: "Do not act as if you had ten thousand years to live.", a: "Marcus Aurelius" },
        { q: "The present is all you have and can control.", a: "Marcus Aurelius" },
        { q: "No man can escape his destiny.", a: "Marcus Aurelius" },
        { q: "It never ceases to amaze me: we all love ourselves more than other people, but care more about their opinions than our own.", a: "Marcus Aurelius" },
        { q: "If you are pained by any external thing, it is not this thing that disturbs you, but your own judgment about it.", a: "Marcus Aurelius" },
        ...
        { q: "A gift consists not in what is done or given, but in the intention of the giver.", a: "Seneca" },
        { q: "If one does not know to which port one is sailing, no wind is favorable.", a: "Seneca" },
        { q: "While we wait for life, life passes.", a: "Seneca" },
        { q: "No man was ever wise by chance.", a: "Seneca" },
        { q: "We are more often frightened than hurt; and we suffer more in imagination than in reality.", a: "Seneca" },
        { q: "Life is long enough, and a sufficiently generous measure has been given to us for the highest achievements.", a: "Seneca" },
        ...
        { q: "We should give as we would receive, cheerfully, quickly, and without hesitation.", a: "Seneca" },
        { q: "The greatest wealth is a mind at peace.", a: "Seneca" },
        { q: "No great genius has existed without an element of madness.", a: "Seneca" },
        { q: "It is a rough road that leads to the heights of greatness.", a: "Lucius Annaeus Seneca" },
        { q: "Fortune is of chameleon-like hue, and the more diligently it is watched, the more quickly it changes.", a: "Seneca" },
        { q: "He who has great power should use it lightly.", a: "Seneca" },
        { q: "Do not spoil what you have by desiring what you have not.", a: "Epicurus" },
        { q: "Of all the means to insure happiness throughout the whole life, by far the most important is the acquisition of friends.", a: "Epicurus" },
        { q: "Not what we have but what we enjoy constitutes our abundance.", a: "Epicurus" },
        { q: "It is not so much our friend's help that helps us, as the confidence of their help.", a: "Epicurus" },
        { q: "The misfortune of the wise is better than the prosperity of the fool.", a: "Epicurus" },
        { q: "Eat and drink with your friends, but do not eat and drink without them.", a: "Epicurus" },
        { q: "He who is not satisfied with a little is satisfied with nothing.", a: "Epicurus" },
        { q: "Empty is the argument of the philosopher which does not relieve any human suffering.", a: "Epicurus" },
        { q: "The art of living well and the art of dying well are one.", a: "Epicurus" },
        { q: "A free life cannot acquire many possessions, because this is not easy to do without servility to mobs or monarchs.", a: "Epicurus" },
        ...
        { q: "A dreamer is one who can find his way by moonlight.", a: "Oscar Wilde" },
        { q: "The truth is rarely pure and never simple.", a: "Oscar Wilde" },
        { q: "The only way to get rid of temptation is to yield to it.", a: "Oscar Wilde" },
        { q: "Life is far too important a thing ever to talk seriously about.", a: "Oscar Wilde" },
        { q: "With freedom, books, flowers, and the moon, who could not be happy?", a: "Oscar Wilde" },
        { q: "The smallest act of kindness is worth more than the grandest intention.", a: "Oscar Wilde" },
        { q: "Every saint has a past, and every sinner has a future.", a: "Oscar Wilde" },
        ...
        { q: "A cynic is a man who knows the price of everything and the value of nothing.", a: "Oscar Wilde" },
        { q: "Keep love in your heart. A life without it is like a sunless garden when the flowers are dead.", a: "Oscar Wilde" },
        { q: "There is no sin except stupidity.", a: "Oscar Wilde" },
        { q: "To love oneself is the beginning of a lifelong romance.", a: "Oscar Wilde" },
        { q: "One should always play fairly when one has the winning cards.", a: "Oscar Wilde" },
        { q: "We are each our own devil, and we make this world our hell.", a: "Oscar Wilde" },
        { q: "Never love anyone who treats you like you're ordinary.", a: "Oscar Wilde" },
        { q: "The best way to appreciate your job is to imagine yourself without one.", a: "Oscar Wilde" },
        { q: "A good friend will always stab you in the front.", a: "Oscar Wilde" },
        { q: "Success is the one unpardonable sin against our fellows.", a: "Oscar Wilde" },
        { q: "There is only one thing in life worse than being talked about, and that is not being talked about.", a: "Oscar Wilde" },
        ...
        { q: "The course of true love never did run smooth.", a: "William Shakespeare" },
        ...
        { q: "One touch of nature makes the whole world kin.", a: "William Shakespeare" },
        ...
        { q: "Our doubts are traitors, and make us lose the good we oft might win, by fearing to attempt.", a: "William Shakespeare" },
        ...
        { q: "The fault, dear Brutus, is not in our stars, but in ourselves.", a: "William Shakespeare" },
        { q: "To mourn a mischief that is past and gone is the next way to draw new mischief on.", a: "William Shakespeare" },
        { q: "This above all: to thine own self be true.", a: "William Shakespeare" },
        { q: "What's past is prologue.", a: "William Shakespeare" },
        { q: "Love looks not with the eyes, but with the mind.", a: "William Shakespeare" },
        { q: "The robbed that smiles steals something from the thief.", a: "William Shakespeare" },
        { q: "Hell is empty and all the devils are here.", a: "William Shakespeare" },
        { q: "The golden age is before us, not behind us.", a: "William Shakespeare" },
        { q: "A fool thinks himself to be wise, but a wise man knows himself to be a fool.", a: "William Shakespeare" },
        ...
        { q: "If we are true to ourselves, we can never go wrong.", a: "William Shakespeare" },
        { q: "We are all fools in love.", a: "William Shakespeare" },
        { q: "Love is blind.", a: "William Shakespeare" },
        { q: "A little more than kin, and less than kind.", a: "William Shakespeare" },
        { q: "It is not in the stars to hold our destiny but in ourselves.", a: "William Shakespeare" },
        { q: "The quality of mercy is not strained.", a: "William Shakespeare" },
        { q: "To sleep: perchance to dream.", a: "William Shakespeare" },
        { q: "There is a special providence in the fall of a sparrow.", a: "William Shakespeare" },
        ...
        { q: "If we cannot make our lives beautiful, we can at least make our lives less ugly.", a: "Virginia Woolf" },
        ...
        { q: "As a woman I have no country. As a woman my country is the whole world.", a: "Virginia Woolf" },
        { q: "Without self-awareness we are as babies in the cradles.", a: "Virginia Woolf" },
        ...
        { q: "It is in our idleness, in our dreams, that the submerged truth sometimes comes to the top.", a: "Virginia Woolf" },
        { q: "No need to hurry. No need to sparkle. No need to be anybody but oneself.", a: "Virginia Woolf" },
        { q: "The eyes of others our prisons; their thoughts our cages.", a: "Virginia Woolf" },
        { q: "A self that goes on changing is a self that goes on living.", a: "Virginia Woolf" },
        { q: "Language is wine upon the lips.", a: "Virginia Woolf" },
        { q: "So long as you write what you wish to write, that is all that matters.", a: "Virginia Woolf" },
        { q: "I would venture to guess that Anon, who wrote so many poems without signing them, was often a woman.", a: "Virginia Woolf" },
        { q: "Books must be read as deliberately and reservedly as they were written.", a: "Virginia Woolf" },
        { q: "The journey is everything.", a: "Virginia Woolf" },
        { q: "No one can think well, love well, sleep well, if one has not dined well.", a: "Virginia Woolf" },
        { q: "A woman must have money and a room of her own if she is to write fiction.", a: "Virginia Woolf" },
        { q: "A man's reach should exceed his grasp, or what's a heaven for?", a: "Robert Browning" },
        { q: "Grow old along with me! The best is yet to be.", a: "Robert Browning" },
        { q: "Less is more.", a: "Robert Browning" },
        { q: "Ah, but a man's reach should exceed his grasp, Or what's a heaven for?", a: "Robert Browning" },
        { q: "God's in his heaven—All's right with the world!", a: "Robert Browning" },
        { q: "The year's at the spring, and day's at the morn.", a: "Robert Browning" },
        { q: "Truth never was indebted to a lie.", a: "Edward Gibbon" },
        { q: "The winds and waves are always on the side of the ablest navigators.", a: "Edward Gibbon" },
        { q: "History is indeed little more than the register of the crimes, follies, and misfortunes of mankind.", a: "Edward Gibbon" },
        { q: "All that is human must retrograde if it does not advance.", a: "Edward Gibbon" },
        { q: "The courage of a soldier is found to be the cheapest of qualities in a battle.", a: "Edward Gibbon" },
        { q: "The history of the world is but the biography of great men.", a: "Thomas Carlyle" },
        { q: "A great man shows his greatness by the way he treats little men.", a: "Thomas Carlyle" },
        { q: "The block of granite which was an obstacle in the pathway of the weak becomes a stepping-stone in the pathway of the strong.", a: "Thomas Carlyle" },
        { q: "Nothing builds self-esteem and self-confidence like accomplishment.", a: "Thomas Carlyle" },
        { q: "Silence is deep as Eternity; speech is shallow as Time.", a: "Thomas Carlyle" },
        { q: "The greatest of faults is to be conscious of none.", a: "Thomas Carlyle" },
        { q: "Our main business is not to see what lies dimly at a distance, but to do what lies clearly at hand.", a: "Thomas Carlyle" },
        { q: "Adversity is the diamond dust Heaven polishes its jewels with.", a: "Thomas Carlyle" },
        { q: "The work an unknown good man has done is like a vein of water flowing hidden underground, secretly making the ground green.", a: "Thomas Carlyle" },
        { q: "Nothing is more terrible than activity without insight.", a: "Thomas Carlyle" },
        { q: "The eternal stars shine out as soon as it is dark enough.", a: "Thomas Carlyle" },
        { q: "The only happiness a brave man ever troubles himself with is to win his own respect.", a: "Thomas Carlyle" },
        { q: "He who has health has hope; and he who has hope has everything.", a: "Thomas Carlyle" },
        { q: "A man without a purpose is like a ship without a rudder.", a: "Thomas Carlyle" }
      ];
      ...
      function renderTodayQuoteItems(data) {
        ...
        if (source) {
          source.textContent = isPt ? "365 frases locais · muda à meia-noite" : "365 local quotes · changes at midnight";
        }
      }
      ...
      async function renderTodayQuote(force) {
        ...
        try {
          ...
          const fb = pickFallbackQuote();
          renderTodayQuoteItems({ q: fb.q, a: fb.a, date: quoteTodayKey(), source: "local", fallback: true });
        } finally {
      ...
      document.getElementById("todayQuoteRefresh")?.addEventListener("click", () => renderTodayQuote(false));
      scheduleQuoteMidnightRefresh();
   ...
   ================================================================ */
      const NOTIF_MESSAGES = {
        ...
        movieDeleted: [
          ...
          { en: "Gone — onto the next pick. 👋", pt: "Saiu — bora escolher outro. 👋" },
        ],
      };
      ...
      function toggleBucketItem(item) {
        if (bucketSyncLive) {
          bucketDb
            .collection("bucketlist")
            .doc(item.id)
            .update({ done: !item.done, updatedAt: firebase.firestore.FieldValue.serverTimestamp() })
            .catch(() => {});
        } else {
          item.done = !item.done;
          renderBucketList();
      ...
      function editBucketItem(item, newText) {
        if (bucketSyncLive) {
          bucketDb
            .collection("bucketlist")
            .doc(item.id)
            .update({ text: newText, updatedAt: firebase.firestore.FieldValue.serverTimestamp() })
   ...
   ================================================================ */
      ...
      function getMoviesByStatus(status) {
        return movieItems
          ...
          .sort((a, b) => {
            if (status === "watched") {
              const aw = a.watchedAt ? (typeof a.watchedAt.toMillis === "function" ? a.watchedAt.toMillis() : Number(a.watchedAt) || 0) : 0;
              const bw = b.watchedAt ? (typeof b.watchedAt.toMillis === "function" ? b.watchedAt.toMillis() : Number(b.watchedAt) || 0) : 0;
      ...
      function renderMovieLists() {
        ...
        if (twCount) twCount.textContent = toWatch.length ? `(${toWatch.length})` : "";
      ...
      function buildMovieCard(item, status) {
        ...
        if (status === "towatch") {
          ...
          toggleBtn.innerHTML = '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M2.5 12s3.5-6 9.5-6 9.5 6 9.5 6-3.5 6-9.5 6-9.5-6-9.5-6Z"/><circle cx="12" cy="12" r="2.5"/></svg>';
          toggleBtn.addEventListener("click", () => setMovieStatus(item, "watched"));
        } else {
          toggleBtn.title = tr("Move back to watchlist");
          toggleBtn.innerHTML = '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M9 6 3 12l6 6"/><path d="M4 12h11a5 5 0 0 1 5 5v1"/></svg>';
        ...
        delBtn.innerHTML = '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 7h16"/><path d="M9 7V4h6v3"/><path d="M7 7l1 13h8l1-13"/><path d="M10 11v5M14 11v5"/></svg>';
      ...
      function setMovieStatus(item, status) {
        ...
        if (movieSyncLive && movieDb) {
          movieDb
            ...
            .then(() => notify(status === "watched" ? "movieWatched" : "movieUpdated"))
            .catch(() => {});
        } else {
      ...
      function rateMovie(item, who, value) {
        ...
        if (movieSyncLive && movieDb) {
          movieDb
            .collection("movies")
            .doc(item.id)
            .update({ ratings, updatedAt: firebase.firestore.FieldValue.serverTimestamp() })
      ...
      async function handleMovieSearch() {
        ...
        if (!isTmdbConfigured()) {
          console.error("TMDB is not configured. Check the TMDB_ACCESS_TOKEN GitHub secret, that GitHub Pages is set to deploy via GitHub Actions (not a branch), and re-run the deploy.");
          if (resultsWrap) {
            resultsWrap.hidden = false;
            resultsWrap.innerHTML = "";
            const p = document.createElement("p");
            p.className = "movie-search-empty";
            p.textContent = "Movie search is not connected yet. Please check the TMDB_ACCESS_TOKEN GitHub secret and that Pages deploys via GitHub Actions.";
          ...
          try {
            ...
            movieUnsub = col.orderBy("createdAt", "asc").onSnapshot(
              ...
              () => {
                movieSyncLive = false;
                setMoviesSyncPill();
                movieItems = JSON.parse(
                  JSON.stringify(CONFIG.movieList || []),
                ).map((it, i) => ({
                  id: "local-" + i,
                  ratings: { me: null, her: null },
                  status: "towatch",
        ...
        if (musicState.editingId) {
          ...
          if (
            ...
            musicDb
              ...
              .update({ ...payload, updatedAt: firebase.firestore.FieldValue.serverTimestamp() })
              .then(() => {
                notify("songUpdated");
                clearMusicAddForm();
                setMusicFormMode(false);
    ...
    `;
      ...
      function saveDaycard(card) {
        ...
        if (
          ...
          daycardsDb
            .collection("daycards")
            .doc(card.id)
            .update({ saved: true, expiresAt: null, updatedAt: firebase.firestore.FieldValue.serverTimestamp() })
            .then(() => {
      ...
      function unsaveDaycard(card) {
        ...
        if (
          ...
          daycardsDb
            .collection("daycards")
            .doc(card.id)
            .update({ saved: false, expiresAt: newExpiry, updatedAt: firebase.firestore.FieldValue.serverTimestamp() })
            .then(() => {
              showToast(
                isPt
      ...
      function tpRunCrashGag(isOldPassword) {
        ...
        function showReveal() {
          ...
          setTimeout(function () { overlay.classList.remove("tp-crash-screenshake"); }, 400);
      ...
      function tpNotesIsUnlocked() {
        try { return sessionStorage.getItem(TP_NOTES_UNLOCK) === "1"; } catch (e) { return false; }
      }
      function tpNotesSetUnlocked() {
        try { sessionStorage.setItem(TP_NOTES_UNLOCK, "1"); } catch (e) {}
      }
      ...
      function tpPomComplete() {
        ...
        tpData.pomodoro.sessions.push({ mode: "timer", minutes: mins, at: new Date().toISOString() });
        tpSave();
        tpPomodoro.left = tpTimerSeconds();
        if (typeof showToast === "function") showToast("Timer complete · " + mins + "m", "updated");
        tpStartAlarm();
      ...
      function tpSwTick() {
        ...
        if (disp) {
          const cur = tpStopwatch.elapsed + (Date.now() - tpStopwatch.startedAt) / 1000;
      ...
      function tpSwReset() {
        ...
        if (mins > 0) {
          ...
          tpData.focusByDay[day] = (tpData.focusByDay[day] || 0) + mins;
          tpData.pomodoro.sessions = tpData.pomodoro.sessions || [];
          tpData.pomodoro.sessions.push({ mode: "stopwatch", minutes: mins, at: new Date().toISOString() });
          tpSave();
          if (typeof showToast === "function") showToast("Session logged · " + mins + "m", "updated");
        }
      ...
      function tpOpenEdit(type, id, meta) {
        ...
        if (type === "dailyGoal") {
          ...
          obj = (tpData.recurringBuys || []).find(function(x){ return x.id === id; });
          if (!obj) return;
          title.textContent = "Edit recurring buy";
          html = tpEditField("Item", "text", obj.title, "title") +
            tpEditField("For", "select", obj.for || "me", "for",
              '<option value="me">For me</option><option value="gift">Gift</option>') +
            tpEditField("Frequency", "select", obj.frequency || "monthly", "frequency",
      ...
      function tpSaveEdit() {
        ...
        if (state.type === "dailyGoal") {
          ...
          obj=(tpData.recurringBuys||[]).find(function(x){return x.id===state.id;});
          if(obj){
      ...
      function tpWeekDayKeys() {
        ...
        for (var i = 6; i >= 0; i--) {
          ...
          out.push({ date: key, label: dd.toLocaleDateString(undefined, { weekday: "short" }) });
        }
        return out;
      }
      function tpBarsHtml(items, max, unit) {
        return items.map(function(d){
          ...
          return '<span data-tp-bar-day="'+tpAttr(d.label+' · '+d.date)+'" data-tp-bar-value="'+tpAttr(value)+'" style="height:'+Math.round((d.n/max)*100)+'%"></span>';
        }).join('');
      }
      function tpViewAnalytics() {
        var days = tpWeekDayKeys();
        var habitsByDay = days.map(function(d){
          var n = tpData.habits.reduce(function(acc,h){ return acc + ((h.log && h.log[d.date]) ? 1 : 0); }, 0);
          return { date: d.date, label: d.label, n: n };
        });
        var goalsByDay = days.map(function(d){
          var list = tpData.dailyGoals[d.date] || [];
          return { date: d.date, label: d.label, n: list.filter(function(g){ return g.done; }).length };
        });
        var habitsWeek = habitsByDay.reduce(function(a,b){ return a+b.n; }, 0);
        var goalsDoneWeek = goalsByDay.reduce(function(a,b){ return a+b.n; }, 0);
        var bestStreak = tpData.habits.reduce(function(m,h){ return Math.max(m, h.best||0); }, 0);
        var maxH = Math.max(1, ...habitsByDay.map(function(x){ return x.n; }));
        var maxG = Math.max(1, ...goalsByDay.map(function(x){ return x.n; }));
        var sessions = (tpData.pomodoro.sessions||[]).slice(-20).reverse();
        return '<div class="tp-grid-3"><div class="tp-stat"><div class="n">'+habitsWeek+'</div><div class="l">Habit checks (7d)</div></div><div class="tp-stat"><div class="n">'+goalsDoneWeek+'</div><div class="l">Goals done (7d)</div></div><div class="tp-stat"><div class="n">'+bestStreak+'</div><div class="l">Best habit streak</div></div></div>' +
          '<div class="tp-card" style="margin-top:14px"><h3>Habits completed — last 7 days</h3><div class="tp-bars">'+tpBarsHtml(habitsByDay, maxH, "habit")+'</div></div>' +
          '<div class="tp-card"><h3>Goals completed — last 7 days</h3><div class="tp-bars">'+tpBarsHtml(goalsByDay, maxG, "goal")+'</div></div>' +
          '<div class="tp-card"><h3>Recent focus sessions</h3>'+(sessions.length?sessions.map(function(s){ return '<div class="tp-item"><strong>'+(s.mode==="stopwatch"?"Stopwatch":"Timer")+'</strong> · '+s.minutes+'m <span style="float:right;font-size:0.72rem;color:var(--ink-soft)">'+(s.at||'').slice(0,16)+'</span></div>'; }).join(''):'<p class="tp-empty">No sessions yet — start one in Focus.</p>')+'</div>';
      }
      ...
      function tpViewPomodoro() {
        ...
        var curSw = tpStopwatch.elapsed + (tpStopwatch.running ? (Date.now()-tpStopwatch.startedAt)/1000 : 0);
        var mS = Math.floor(curSw/60), sS = Math.floor(curSw%60);
        var dispS = String(mS).padStart(2,"0")+":"+String(sS).padStart(2,"0");
        var tabsHtml = '<div class="tp-modes">' +
          '<button type="button" class="tp-btn outline '+(tab==="timer"?"active":"")+'" data-tp-focus-tab="timer">Timer</button>' +
          '<button type="button" class="tp-btn outline '+(tab==="stopwatch"?"active":"")+'" data-tp-focus-tab="stopwatch">Stopwatch</button>' +
          '</div>';
        var bodyHtml;
        if (tab === "stopwatch") {
          bodyHtml = '<h3>Stopwatch</h3><p class="tp-sub">Track time with no set limit · resetting logs it</p>' +
            '<div class="tp-timer" id="tpSwDisp">'+dispS+'</div><div class="tp-row" style="justify-content:center">' +
            '<button type="button" class="tp-btn" id="tpSwStart">'+(tpStopwatch.running?"Pause":"Start")+'</button>' +
            '<button type="button" class="tp-btn outline" id="tpSwReset">Reset</button></div>';
        } else {
          bodyHtml = '<h3>Timer</h3><p class="tp-sub">Set a duration and focus</p>' +
            (!tpPomodoro.running ? '<div class="tp-row" style="justify-content:center;align-items:center;gap:8px"><input id="tpTimerMins" type="number" min="1" max="180" value="'+(tpData.pomodoro.minutes||25)+'" style="max-width:90px;text-align:center"><span class="tp-sub" style="margin:0">minutes</span></div>' : '') +
            '<div class="tp-timer" id="tpTimerDisp">'+dispT+'</div><div class="tp-row" style="justify-content:center">' +
...
function tpViewWorkout() {
      ...
      function tpViewNotes() {
        ...
        return '<div class="tp-card tp-notes-card">' + tpCardHead("Notes", "A private space to scribble anything — ideas, plans, reminders, thoughts.", "notes") +
          (edit ? (tpFmtBar("#tpBigNotes") +
            '<textarea id="tpBigNotes" class="tp-notes tp-big-notes" placeholder="Write anything here… **bold** *italic* - bullets · 1. numbered lists">' + tpAttr(val) + '</textarea>' +
            ...
            '<div class="tp-notes-view tp-md">' + (val ? tpMd(val) : '<span class="tp-notes-empty">No notes yet. Click Edit to start writing.</span>') + '</div>') +
          '</div>';
      }
      ...
      function tpBindChartTooltips() {
        ...
        document.querySelectorAll("[data-tp-bar-day]").forEach(function(el){
          ...
          el.addEventListener("touchstart", function(e){ e.preventDefault(); showTip(el); }, { passive: false });
        });
        document.addEventListener("touchstart", function(e){
          if (!e.target.closest || !e.target.closest("[data-tp-bar-day]")) hideTip();
        }, { passive: true });
      }
      function tpBindView() {
        var notesUnlockBtn = document.getElementById("tpNotesUnlockBtn");
        if (notesUnlockBtn) notesUnlockBtn.addEventListener("click", function(){
          var input = document.getElementById("tpNotesPassInput");
          var err = document.getElementById("tpNotesPassErr");
          if (((input || {}).value || "") === TP_NOTES_PASS) {
            tpNotesSetUnlocked();
        ...
        if (notesPassInput) notesPassInput.addEventListener("keydown", function(e) {
        ...
        if (saveNotes) saveNotes.addEventListener("click", function(){
          ...
          if (saved) { saved.textContent = "Saved · private · password protected"; setTimeout(function(){ if(saved) saved.textContent = "Private · password protected"; }, 1800); }
        });
        tpBindChartTooltips();
        ...
        if(swReset) swReset.addEventListener("click", function(){ tpSwReset(); });
        ...
        if(addRecurring) addRecurring.addEventListener("click", function(){
          var title=((document.getElementById("tpRecTitle")||{}).value||"").trim();
          if(!title) return;
          tpData.recurringBuys.push({
            id:tpUid(),
            title:title,
            for:((document.getElementById("tpRecFor")||{}).value)||"me",
            frequency:((document.getElementById("tpRecFreq")||{}).value)||"monthly",
        ...
        document.querySelectorAll("[data-tp-del-recurring]").forEach(function(b){
          b.addEventListener("click", function(){
            tpData.recurringBuys=tpData.recurringBuys.filter(function(x){return x.id!==b.getAttribute("data-tp-del-recurring");});
            tpSave(); tpRender();
          });
        });
      ...
      window.addEventListener("beforeunload", function(){ tpStopAlarm(); });
      initThommyPersonal();

      // ================================================================
      // PHOTO BOOTH — live two-camera duo view (WebRTC) + synced strip
      // Signaling only lives in Firestore just long enough to connect
      // (collection "photoboothCalls"); saved strips are optional and
      // auto-expire after 24h (collection "photoboothStrips").
      // ================================================================
      (function () {
        ...
        // above. Getting your own takes 5 minutes — see METERED_TURN_SETUP.md
          ...
          if (isMeteredTurnConfigured()) {
            try {
              ...
              try {
                ...
                errorMessage =
                  errorJson.error || errorJson.message || bodyText;
              } catch (_) {}
              console.warn(
        ...
        function pbRenderAllSwatches() {
          ...
          swatchWrap.querySelectorAll("canvas").forEach((c) => pbRenderPatternSwatch(c, c.dataset.pattern));
        }
        ...
        function pbSetSyncPill(state) {
          ...
          if (state === "connected") {
            ...
            pill.textContent = pbT("🟡 Not connected yet — start the camera", "🟡 Ainda não conectado — ligue a câmera");
            pill.className = "bucket-sync-pill local";
          }
        }
        ...
        function pbFirestoreErrorText(e) {
          ...
          return pbT(
            "Sync error: " + (code || (e && e.message) || "unknown") + ". Check the Firebase console/Firestore rules.",
            "Erro de sincronização: " + (code || (e && e.message) || "desconhecido") + ". Verifica o Firebase console/regras do Firestore.",
          );
        }
        ...
        function pbBuildStaticUI() {
          ...
          if (swatchWrap && !swatchWrap.dataset.ready) {
            ...
            Object.keys(PB_PATTERNS).forEach((id) => {
              ...
              const btn = document.createElement("button");
              btn.type = "button";
              btn.className = "pb-theme-swatch" + (id === pbPatternId ? " active" : "");
              ...
              btn.addEventListener("click", () => {
                ...
                swatchWrap
                  ...
                  .forEach((s) => s.classList.toggle("active", s.dataset.pattern === id));
          ...
          if (posesSelect && !posesSelect.dataset.ready) {
            ...
            posesSelect.addEventListener("change", async () => {
              ...
              if (pbRoomMode === "create" && pbCallId) {
                try {
                  const db = pbGetDb();
                  if (db) await db.collection("photoboothCalls").doc(pbCallId).set({ poseCount: pbPoseCount }, { merge: true });
                } catch (e) { console.warn("Could not sync booth pose count", e); }
              }
            });
          }
        }
        ...
        async function pbNewPeerConnection() {
          ...
          if (pbLocalStream) {
            pbLocalStream.getTracks().forEach((track) => pc.addTrack(track, pbLocalStream));
          }
          ...
          pc.onconnectionstatechange = () => {
            if (pc.connectionState === "connected") {
              ...
              if (status) {
                status.hidden = false;
                status.textContent = pbT("Connected — loading partner camera…", "Conectado — a carregar a câmara do seu par…");
                setTimeout(() => {
          ...
          pc.oniceconnectionstatechange = () => {
            if (["failed", "disconnected"].includes(pc.iceConnectionState) && !pbConnected) {
              const status = pbEl("pbRemoteStatus");
              if (status) {
                status.hidden = false;
                status.textContent = pbT(
          ...
          pc.onicecandidateerror = (event) => {
            console.warn("Photo booth ICE candidate error", event && event.errorText, event);
          };
          return pc;
        }
        ...
        function pbScheduleConnectWatchdog(ref) {
          ...
          pbConnectWatchdogTimer = setTimeout(async () => {
            ...
            if (pbRole === "caller") {
              ...
              pbSetSyncPill("waiting");
              pbCleanupSignaling();
              try { await pbResetCallDoc(ref); await pbStartAsCaller(ref); }
              catch (e) { console.warn("Photo booth caller retry failed", e); }
            } else {
              const status = pbEl("pbRemoteStatus");
              if (status) {
                status.hidden = false;
        ...
        function pbSetupDataChannelHandlers(channel) {
          ...
          channel.onopen = () => {
            if (pbRole === "caller") {
              pbSendMessage({ type: "config", poses: pbPoseCount, filter: pbFilterId });
            }
          };
        ...
        async function pbStartAsCaller(ref) {
          ...
          pbPc.onicecandidate = (event) => {
            if (event.candidate) {
              callerCands.add(event.candidate.toJSON()).catch((e) => pbShowIssue(pbFirestoreErrorText(e)));
            }
          };
          ...
          pbUnsubCall = ref.onSnapshot((snap) => {
          ...
          pbUnsubCalleeCands = ref.collection("calleeCandidates").onSnapshot((snap) => {
            snap.docChanges().forEach((change) => {
              if (change.type === "added" && pbPc) {
                pbPc.addIceCandidate(new RTCIceCandidate(change.doc.data())).catch(() => {});
              }
            });
          }, (e) => pbShowIssue(pbFirestoreErrorText(e)));
        ...
        async function pbStartAsCallee(ref, offerData) {
          ...
          pbPc.onicecandidate = (event) => {
            if (event.candidate) {
              calleeCands.add(event.candidate.toJSON()).catch((e) => pbShowIssue(pbFirestoreErrorText(e)));
            }
          };
          ...
          try {
            await ref.update({ answer: { type: answer.type, sdp: answer.sdp }, status: "connecting" });
          } catch (e) {
            pbShowIssue(pbFirestoreErrorText(e));
            throw e;
          }

          pbUnsubCallerCands = ref.collection("callerCandidates").onSnapshot((snap) => {
            snap.docChanges().forEach((change) => {
              if (change.type === "added" && pbPc) {
                pbPc.addIceCandidate(new RTCIceCandidate(change.doc.data())).catch(() => {});
              }
        ...
        async function pbJoinCall() {
          ...
          const db = pbGetDb();
          if (!db || !pbCallId) { pbSetSyncPill(db ? "waiting" : "unavailable"); return; }
          ...
          pbUnsubCall = ref.onSnapshot(async (snap) => {
            const data = snap.exists ? snap.data() : null;
            if (!data) return;
            if (data.expiresAt && Number(data.expiresAt) < Date.now()) return;
            if (data.answer || pbRole === "callee" || pbConnected || !data.offer || !pbCameraOn) return;
            try { await pbStartAsCallee(ref, data.offer); }
        ...
        function pbClearSoloTimer() {
          if (pbSoloTimer) { clearTimeout(pbSoloTimer); pbSoloTimer = null; }
        }
        ...
        async function pbCreateRoom() {
          ...
          const err = pbEl("pbRoomError");
          if (!db) {
            if (err) err.textContent = pbT("Shared booths need Firebase to be configured on the site.", "As cabines partilhadas precisam do Firebase configurado no site.");
            return;
          }
          const btn = pbEl("pbCreateRoomBtn");
          if (btn) { btn.disabled = true; btn.textContent = pbT("Creating…", "A criar…"); }
          try {
            let code = null;
            for (let i = 0; i < 8; i++) {
              const candidate = pbMakeRoomCode();
              const snap = await db.collection("photoboothCalls").doc(candidate).get();
            ...
            await db.collection("photoboothCalls").doc(code).set({
              status: "waiting",
              poseCount: pbPoseCount,
              createdAt: firebase.firestore.FieldValue.serverTimestamp(),
              expiresAt: Date.now() + PB_CALL_TTL_MS,
            ...
            if (err) err.textContent = pbT("Couldn't create a booth. Please try again.", "Não foi possível criar a cabine. Tenta novamente.");
          } finally {
            if (btn) { btn.disabled = false; btn.textContent = pbT("Create a new booth", "Criar uma nova cabine"); }
          }
        }

        async function pbJoinRoom() {
          const input = pbEl("pbRoomCodeInput");
          const code = String((input && input.value) || "").trim().toUpperCase().replace(/[^A-Z0-9]/g, "");
          const err = pbEl("pbRoomError");
          if (code.length !== 6) {
            if (err) err.textContent = pbT("Enter the 6-character booth code.", "Introduz o código de 6 caracteres.");
            input && input.focus();
            return;
          }
          const db = pbGetDb();
          ...
          if (btn) { btn.disabled = true; btn.textContent = pbT("Checking…", "A verificar…"); }
          try {
            const ref = db.collection("photoboothCalls").doc(code);
            const snap = await ref.get();
            if (!snap.exists) {
              if (err) err.textContent = pbT("That booth doesn't exist. Ask the creator for the current code.", "Essa cabine não existe. Pede à pessoa que criou a cabine o código atual.");
              return;
            }
            const data = snap.data() || {};
            if (data.expiresAt && Number(data.expiresAt) < Date.now()) {
              if (err) err.textContent = pbT("That booth code has expired. Ask the creator for a new one.", "Esse código de cabine expirou. Pede à pessoa que criou a cabine um novo.");
              return;
            }
            if (data.status && data.status !== "waiting" && data.status !== "connecting") {
              if (err) err.textContent = pbT("That booth is no longer available.", "Essa cabine já não está disponível.");
              return;
            }
            if (data.answer) {
              if (err) err.textContent = pbT("That booth is already full.", "Essa cabine já está cheia.");
              return;
            }
            // Guest inherits the host's pose count and cannot change it.
            pbPoseCount = [3, 4, 6].includes(Number(data.poseCount)) ? Number(data.poseCount) : 4;
            pbCallId = code;
            pbRoomMode = "join";
            pbSoloMode = false;
            const posesSelect = pbEl("pbPosesSelect");
            if (posesSelect) { posesSelect.value = String(pbPoseCount); posesSelect.disabled = true; }
            ...
            if (err) err.textContent = pbT("Couldn't check that booth. Please try again.", "Não foi possível verificar essa cabine. Tenta novamente.");
          } finally {
            if (btn) { btn.disabled = false; btn.textContent = pbT("Join booth", "Entrar na cabine"); }
          }
        }
        ...
        function pbUpdateTakeBtnEnabled() {
          ...
          if (!btn) return;
          btn.disabled =
            !(pbConnected || pbSoloMode) || pbCountdownActive || pbShots.length >= pbPoseCount;
        }
        ...
        function pbRenderStripPreview() {
          ...
          canvas.height =
            pad + pbShotImages.length * displayH + (pbShotImages.length - 1) * gap + pad + captionH;
          const ctx = canvas.getContext("2d");
          ctx.fillStyle = pbBgColor;
          ...
          pbPaintPattern(ctx, pbPatternId, pbAccentColor, canvas.width, canvas.height);
          ...
          if (caption) {
            ...
            ctx.fillText(
              (CONFIG.names && CONFIG.names.me && CONFIG.names.her
                ? CONFIG.names.me + " ♥ " + CONFIG.names.her
                : "Photo Booth") + " · " + new Date().toLocaleDateString(),
              canvas.width / 2,
              canvas.height - 10,
            );
            ctx.globalAlpha = 1;
        ...
        function pbBindOnce() {
          ...
          if (roomCodeInput) roomCodeInput.addEventListener("input", () => {
            roomCodeInput.value = roomCodeInput.value.toUpperCase().replace(/[^A-Z0-9]/g, "").slice(0, 6);
          });
          if (roomCodeInput) roomCodeInput.addEventListener("keydown", (e) => {
          ...
          if (roomContinueBtn) roomContinueBtn.addEventListener("click", () => pbShowCameraSetup("create"));
          const roomBackBtn = pbEl("pbRoomBackBtn");
          if (roomBackBtn) roomBackBtn.addEventListener("click", () => {
            pbCallId = null; pbRoomMode = null;
          ...
          if (captionInput) captionInput.addEventListener("input", pbRenderStripPreview);
        }
